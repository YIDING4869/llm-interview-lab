...
import { mockInterviewTracks, resolveMockInterviewQuestion } from '../../data/mock-interviews';
...
export function MockInterviewWorkspace() {
  ...
  const questions = useMemo(() => activeTrack.questions.map(resolveMockInterviewQuestion).filter((item) => item !== null), [activeTrack]);
  ...
  const startSession = (trackId = selectedTrackId) => {
    ...
    const nextSession: MockSession = { id, trackId, startedAt: new Date().toISOString(), currentIndex: 0, stage: 'main', answers: {} };
    setStorage((current) => ({ ...current, active: nextSession }));
    setSelectedTrackId(trackId);
    setSecondsLeft(90);
    setTimerRunning(true);
    setStatus('第 1 道主问题已开始；先给结论，再展开证据');
    saveLastLearningActivity({ type: 'mock', trackId });
    trackEvent('practice_start', { surface: 'mock_interview_main', track_id: trackId, question_index: 1 });
  };
  ...
  const saveMain = () => {
    ...
    trackEvent('practice_complete', { surface: 'mock_interview_main', track_id: session.trackId, question_index: session.currentIndex + 1, answer_length: answer.length });
    trackEvent('practice_start', { surface: 'mock_interview_followup', track_id: session.trackId, question_index: session.currentIndex + 1 });
  };
  ...
  const moveAfterFollowup = (followupPatch: Partial<MockAnswer> = {}) => {
    ...
    saveLastLearningActivity({ type: 'mock', trackId: session.trackId });
    if (!isLast) trackEvent('practice_start', { surface: 'mock_interview_main', track_id: session.trackId, question_index: session.currentIndex + 2 });
  };
  ...
  const saveFollowup = () => {
    ...
    if (!answer) return;
    trackEvent('practice_complete', { surface: 'mock_interview_followup', track_id: session.trackId, question_index: session.currentIndex + 1, answer_length: answer.length });
    ...
    <main className="mock-page">
      ...
      {!session && <>
        ...
        <section className="mock-track-section" aria-busy={!hydrated}>
          <div className="mock-track-head"><div><span>CHOOSE A ROLE</span><h2>选择最接近目标岗位的一场。</h2></div><p>每套题固定取自站内可追溯面经，顺序从基础机制走向项目证据；开始后仍可提前结束并查看真实缺口。</p></div>
          ...
          <div className="mock-start-bar"><div><span>READY</span><strong>{selectedTrack.title}</strong><small>约 12 分钟 · 可暂停 · 可提前结束 · 本机保存</small></div><button type="button" disabled={!hydrated || questions.length !== 5} onClick={() => startSession()}>开始整场模拟 <span>→</span></button></div>
      ...
      {session && session.stage !== 'report' && currentQuestion && <section className="mock-session">
        <header className="mock-session-head"><div><a href={sitePath('/mock/')}>模拟面试</a><span>/</span><strong>{activeTrack.title}</strong></div><button type="button" onClick={finishNow}>提前结束并复盘</button></header>
      ...
      {session?.stage === 'report' && currentReport && <section className="mock-report">
        <header><div><span>LOCAL MOCK REPORT</span><h1>{activeTrack.title}<br /><em>本机复盘报告</em></h1><p>这不是模型评分。它只汇总你实际完成的回答、表达长度和自评，用于决定下一轮练什么。</p></div><div><strong>{currentReport.mainCompleted}<b>/5</b></strong><span>主问题完成</span><strong>{currentReport.followupsCompleted}<b>/5</b></strong><span>追问完成</span></div></header>
        ...
        <div className="mock-report-actions"><button type="button" onClick={() => startSession(session.trackId)}>再练同一场 <span>→</span></button><button type="button" onClick={clearActive}>更换岗位方向</button><a href={sitePath('/progress/')}>查看全部本机进度 →</a></div>
      </section>}
