...
export default async function QuestionPage({ params }: QuestionPageProps) {
  const { questionId } = await params;
  const question = practiceQuestions.find((item) => item.id === Number(questionId));
  if (!question) return <main className="content-detail-page"><SiteHeader active="questions" /><section className="content-detail-missing"><h1>没有找到这道题</h1><a href={sitePath('/#question-bank')}>返回题库 →</a></section><SiteFooter /></main>;
  ...
  return (
    <main className="content-detail-page question-detail-page">
      ...
      <article className="content-detail-shell">
        <nav className="page-breadcrumb"><a href={sitePath('/')}>首页</a><span>→</span><a href={sitePath('/#question-bank')}>面试题</a><span>→</span><strong>Q{question.id.toString().padStart(2, '0')}</strong></nav>
        <header className="content-detail-hero">
          ...
          <div className="content-detail-actions"><a className="primary-button" href={sitePath(`/practice/?module=${question.moduleId}&question=${question.id}`)}>进入限时作答 <span>→</span></a>{knowledgeModule && <a href={sitePath(`/learn/#module-${knowledgeModule.id}`)}>查看 {knowledgeModule.title} 知识模块 ↗</a>}</div>
        </header>
        ...
        <section className="question-detail-reflect"><span>REFLECT IN YOUR OWN WORDS</span><blockquote>{question.notePrompt}</blockquote><a href={sitePath(`/practice/?module=${question.moduleId}&question=${question.id}`)}>保存自己的答案与复盘 →</a></section>
      </article>
      <SiteFooter />
    </main>
  );
