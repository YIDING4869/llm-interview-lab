...
export type MockInterviewTrack = {
  ...
  description: string;
  tags: string[];
  questions: MockInterviewQuestionSpec[];
};
...
export function resolveMockInterviewQuestion(spec: MockInterviewQuestionSpec): ResolvedMockInterviewQuestion | null {
  ...
  const followup = guide?.followups[spec.followupIndex];
  return record && prompt && guide && followup ? { ...spec, record, prompt, guide, followup } : null;
}
