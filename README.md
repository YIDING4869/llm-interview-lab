...
- 13 份可追溯公开面试流程与 52 道改写真题；22 道校准题在首答后解锁分层参考，并可继续完成 60 秒连续追问
- 五个岗位方向、每个方向三套自动轮换的 12 分钟模拟面试，每场包含 5 道主问题、5 道追问、本机结构化复盘、最近三场自评趋势和下一场训练处方
- 按目标岗位、面试日期和每日 20/45/90 分钟预算倒排的冲刺计划，自动切换基础补齐、专项作答、整场模拟和最后复盘阶段
- 响应式布局和 PWA manifest
...
- `/plan/`：生成未来七天的面试倒计时任务，并在三天以内自动进入紧急冲刺
- `/interviews/`：可搜索、按方向筛选并限时作答的真题训练器；可分别练知识机制和项目深挖，并在首答后进入 60 秒追问模拟
- `/mock/`：模型基础、RAG/Agent、AI Infra、多模态和项目深挖五个方向的轮换题单，以及仅保存在本机的复盘报告
